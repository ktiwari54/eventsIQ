...
function LogoutFooter({ email }: { email?: string | null }) {
  return (
    <div className="shrink-0 border-t border-border p-3">
      <div className="text-[11px] text-muted truncate mb-2 px-1">{email}</div>
      <button
    ...
    <>
      ...
      {open && (
        <div className="md:hidden">
          ...
          />
          {/* Drawer */}
          <aside className="fixed top-0 left-0 h-screen w-[240px] bg-card border-r border-border flex flex-col z-50">
            ...
            <LogoutFooter email={session?.user?.email} />
          </aside>
        </div>
      )}

      {/* ── Desktop sidebar (always visible) ── */}
      <aside className="hidden md:flex fixed top-0 left-0 h-screen w-[210px] bg-card border-r border-border flex-col z-50">
